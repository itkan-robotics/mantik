import type { ArmPlantConfig, LintMessage, ParseResult, TuningConfig, Vendor } from '../types';
import { DEFAULT_TUNING } from '../types';
import { armTuningWarnings } from '../reference/armReference';
import { revArmTemplate } from '../templates/revArm';
import { ctreArmTemplate } from '../templates/ctreArm';
import { revArmRobotTemplate } from '../templates/revArmRobot';
import { ctreArmRobotTemplate } from '../templates/ctreArmRobot';
...
export function parseArmCode(source: string, vendor: Vendor): ParseResult {
  ...
  if (configForWarnings) {
    for (const msg of armTuningWarnings(configForWarnings, vendor)) {
...
export function getArmTemplateForVendor(
  vendor: Vendor,
  config: TuningConfig,
  plant: ArmPlantConfig,
): string {
  return vendor === 'rev' ? revArmTemplate(config, plant) : ctreArmTemplate(config, plant);
}

export function getArmRobotTemplateForVendor(vendor: Vendor): string {
  return vendor === 'rev' ? revArmRobotTemplate() : ctreArmRobotTemplate();
}
