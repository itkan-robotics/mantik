...
**Browser practice:** Open [PID Simulation](/pid-simulation) on Mantik and choose **Arm** plus your motor vendor. Setpoint and kP use motor rotations (V/rot); the Mechanism view and TraceView show degrees. The browser plant matches [controls_js_sim](https://github.com/wpilibsuite/wpilib-docs/tree/main/source/_extensions/controls_js_sim) (5 kg, 1 m, 100:1). Gravity feedforward uses kG · cos(angle). Retune kP in the browser even if you watched the lesson video on YAMS hardware.
...
1. Set the setpoint to a position within the arm's range. Watch for the red line (soft limit) showing the maximum allowed position. Avoid setting the setpoint too close to or beyond it, as this can cause glitches in simulation. Find a position that works well for your arm mechanism.
2. Find the smallest kG value that holds position without moving:
...
4. Start tuning kP using the doubling method:
   - Begin with kP = 0.1
...
6. Configure motion profiling in the arm subsystem code. Find the closed loop controller settings (motion profiling constraints):
...
9. Fine-tune motion profile and kP together:
...
10. Evaluate the final motion profile. The ideal motion should look rectangular/boxy in the position graph:
...
11. Compare before and after motion profiling. Without motion profiling, the position graph may show erratic movement that takes longer and is less efficient. With proper tuning, you should see clean, rectangular transitions.

</StepsBox>

<RulesBox title="Arm Tuning Process Summary" subtitle="Complete workflow:">
...
<LinkGrid links={[{"label":"Try in browser","url":"/pid-simulation"},{"label":"PID Control","id":"frc-pid-control"},{"label":"PID Tuning Practice - Elevator","id":"pid-tuning-practice-elevator"},{"label":"Trapezoidal Motion Profiling","id":"trapezoidal-motion-profiling"},{"label":"Feedforward Control","url":"https://docs.wpilib.org/en/stable/docs/software/advanced-controls/controllers/feedforward-control.html"}]} title="Resources" section="frc" />
